...
data "aws_ami" "latest_amazon_linux" {
  ...
  filter {
    ...
    values = [var.nat_ami_type] # Using ARM for cost optimization
  }

  filter {
    name = "virtualization-type"
    # values = ["hvm"]
    values = [var.nat_ami_virtualization_type]
  }
...
resource "aws_instance" "nat_ec2_instance" {
  ...
  # user_data = filebase64("${path.cwd}/iptables.sh")
  key_name               = var.nat_bastion_key_name
