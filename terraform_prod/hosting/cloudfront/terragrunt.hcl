...
terraform {
  ...
  source = "tfr://gitlab.com/arsalanshaikh13/tf-modules-lirw-packer/aws//hosting/cloudfront?version=1.1.0-domain-change"
...
dependency "waf" {
  ...
  config_path                             = "${dirname(get_terragrunt_dir())}/waf"
  mock_outputs                            = include.global_mocks.locals.global_mock_outputs
  mock_outputs_allowed_terraform_commands = ["plan"]
}
...
inputs = {
  ...
  cloudfront_waf_arn  = dependency.waf.outputs.cloudfront_waf_arn
}
