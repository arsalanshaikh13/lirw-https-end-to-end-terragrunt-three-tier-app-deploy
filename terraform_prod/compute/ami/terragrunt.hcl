...
locals {
  ...
  frontend_manifest = "${local.packer_folder}/frontend/manifest-${local.environment}.json"
  backend_manifest  = "${local.packer_folder}/backend/manifest-${local.environment}.json"
}

terraform {
  # source = "../../../../modules/app"
  # source = "${path_relative_from_include("root")}/modules/compute/ami"
  source = "tfr://gitlab.com/arsalanshaikh13/tf-modules-lirw-packer/aws//compute/ami?version=1.0.1-ami-region"
