...
server_key_name=$(terragrunt --working-dir ../../terraform/nat_key/key output  -raw server_key_name)
key_file_path=../../modules/nat_key/key/server_key
...
# save and increment the packerlog file name for each new log
LOG_DIR="packer-logs"
mkdir -p "$LOG_DIR"
...
cp "$LOG_FILE" "$LOG_FILE.bak"   # Backup raw log

# Strip ANSI escape sequences
sed -E 's/\x1b\[[0-9;]*m//g' "$LOG_FILE.bak" > "$LOG_FILE"
# sed -r "s/\x1B\[[0-9;]*[A-Za-z]//g" "$LOG_FILE.bak" > "$LOG_FILE"
