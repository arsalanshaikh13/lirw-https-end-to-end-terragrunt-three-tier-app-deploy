...
5. ancestor_based_operation [FLAGS] <ancestor_pattern> <src> <dst> [file_name]
  <ancestor_pattern> : folder_name(glob-pattern) or folder_name(glob-pattern)/folder_name(glob-pattern) only 1 and 2 levels
  <src> and <dst> : same as ancestor pattern but any levels
