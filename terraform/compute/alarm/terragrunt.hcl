...
terraform {
  # source = "../../../../modules/app"
  # source = "${path_relative_from_include("root")}/modules/compute/alarm"
  source = "tfr://gitlab.com/arsalanshaikh13/tf-modules-lirw-packer/aws//compute/alarm?version=1.0.0-lirw-packer"
...
dependency "asg" {
  # config_path                             = "../../asg/vpc"
  config_path                             = "${dirname(get_terragrunt_dir())}/asg"
  mock_outputs                            = include.global_mocks.locals.global_mock_outputs
  mock_outputs_allowed_terraform_commands = ["plan"]
}
...
inputs = {
  ...
  client_asg_name = dependency.asg.outputs.client_asg_name
  server_asg_name = dependency.asg.outputs.server_asg_name
}
